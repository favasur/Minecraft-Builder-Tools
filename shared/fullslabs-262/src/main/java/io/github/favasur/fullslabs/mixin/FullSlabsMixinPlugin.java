...
/**
 * 26.2 FullSlabs mixin plugin: gates client-only mixins (e.g. {@code BlockStateModelSetMixin})
 * on dedicated servers, where the client classes are absent, and skips any target that cannot be
 * loaded on the current side. Every mixin in the config is otherwise adapted to the 26.2 API.
 */
