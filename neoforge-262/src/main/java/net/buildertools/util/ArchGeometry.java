...
public final class ArchGeometry {
     ...
     * of the wall stays occupied (the far side of the wall swings toward the click). The bend
     * is split into {@code count} equal angular slices of {@code T/count} (T = the total bend
     * angle); the first wedge is expected to sit exactly on the pivot so it keys the first
     * cell and the last keys the farthest cell.
     */
     ...
     * textures on the joints and ends), tiling once per meter (the {@link #uv} helper wraps the
     * tile coordinates so the texture never samples past the sprite's atlas rectangle).
     */
    public static List<BakedQuad> wedgeQuads(ArchBlockData a, BlockState state) {
        ...
        for (WedgeFace face : wedgeFaces(a)) {
            Vec3[] c = face.corners();
            BakedQuad.MaterialInfo info = materialFor(state, spriteDirs[face.kind()]);
            if (info == null || info.sprite() == null) {
                continue;
            }
            Direction cull = nearestDirection(outwardNormal(c[0], c[1], c[2]));
            addFaceQuads(quads, face, info, cull);
        }
        return quads;
    }
    ...
    static void addFaceQuads(List<BakedQuad> out, WedgeFace face,
                             BakedQuad.MaterialInfo info, Direction cull) {
        ...
        if (boundary > base && maxU > boundary) {
            ...
            emitQuad(out, c[ai[0]], s0, s1, c[ai[1]],
                    lowU, 1.0, 1.0, lowU,
                    face.v()[ai[0]], face.v()[ai[0]], face.v()[ai[1]], face.v()[ai[1]], sprite, cull, info);
            ...
            emitQuad(out, c[0], c[1], c[2], c[3],
                    ...
                    face.v()[0], face.v()[1], face.v()[2], face.v()[3], sprite, cull, info);
        }
    }
    ...
    private static void emitQuad(List<BakedQuad> out, Vec3 p0, Vec3 p1, Vec3 p2, Vec3 p3,
                                 ...
                                 TextureAtlasSprite sprite, Direction cull, BakedQuad.MaterialInfo info) {
        out.add(new BakedQuad(
                vec(p0), vec(p1), vec(p2), vec(p3),
                uv(sprite, (float) u0, (float) v0), uv(sprite, (float) u1, (float) v1),
                uv(sprite, (float) u2, (float) v2), uv(sprite, (float) u3, (float) v3),
                cull, info));
    }

    static Vector3f vec(Vec3 p) {
    ...
    static Vec3 outwardNormal(Vec3 p0, Vec3 p1, Vec3 p2) {
    ...
    /** The dominant axis direction of a unit normal (26.2 removed Direction.getNearest). */
    static Direction nearestDirection(Vec3 n) {
        double ax = Math.abs(n.x);
        double ay = Math.abs(n.y);
        double az = Math.abs(n.z);
        if (ax >= ay && ax >= az) {
            return n.x >= 0.0 ? Direction.EAST : Direction.WEST;
        }
        if (ay >= az) {
            return n.y >= 0.0 ? Direction.UP : Direction.DOWN;
        }
        return n.z >= 0.0 ? Direction.SOUTH : Direction.NORTH;
    }

    /**
     * The block's material info (sprite + render type + tint/shade flags) for the given face,
     * taken from a real quad of the block's model so the wedge textures match the block exactly;
     * falls back to the model's particle material when the face has no quads. Null when the model
     * is not loaded.
     */
    @Nullable
    static BakedQuad.MaterialInfo materialFor(BlockState state, Direction face) {
        ...
        try {
            ...
            if (model != null) {
                ...
                for (BlockStateModelPart part : parts) {
                    ...
                    if (!quads.isEmpty() && quads.get(0).materialInfo() != null
                            && quads.get(0).materialInfo().sprite() != null) {
                        return quads.get(0).materialInfo();
                    }
                    if (fallback == null) {
                        for (Direction d : Direction.values()) {
                            List<BakedQuad> any = part.getQuads(d);
                            if (!any.isEmpty() && any.get(0).materialInfo() != null
                                    && any.get(0).materialInfo().sprite() != null) {
                                fallback = any.get(0).materialInfo();
                                break;
                            }
                        }
                        if (fallback == null) {
                            List<BakedQuad> any = part.getQuads(null);
                            if (!any.isEmpty() && any.get(0).materialInfo() != null
                                    && any.get(0).materialInfo().sprite() != null) {
                                fallback = any.get(0).materialInfo();
                            }
                ...
                if (fallback == null) {
                    ...
                    if (particle != null && particle.sprite() != null) {
                        return BakedQuad.MaterialInfo.of(particle, particle.sprite().transparency(),
                                -1, true, 0);
                    }
                }
                return fallback;
            ...
            // Missing/unloaded model: fall through.
        }
        return null;
    }
}
