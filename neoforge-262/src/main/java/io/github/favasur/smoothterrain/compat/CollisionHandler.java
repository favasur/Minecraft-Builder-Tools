 ...
 * - Entities shouldn't fall through the floor ({@link io.github.favasur.smoothterrain.mixin.BlockStateBaseMixin#noCubes$getCollisionShape})
 * - Player should be able to walk into and out of single block holes ({@link io.github.favasur.smoothterrain.mixin.BlockStateBaseMixin#noCubes$hasLargeCollisionShape})
 * - Player should be able to walk up slopes made out of normally solid blocks like stone/dirt/grass/sand ({@link io.github.favasur.smoothterrain.mixin.BlockStateBaseMixin#noCubes$hasLargeCollisionShape})
 * - Player should be able to swim through smooth blocks underwater without suffocating (e.g. near the shore) ({@link io.github.favasur.smoothterrain.mixin.BlockCollisionsMixin#noCubes$isSuffocating})
 ...
 * - 3rd person camera should not be super zoomed-in when only partially inside a smooth voxel ({@link io.github.favasur.smoothterrain.mixin.BlockStateBaseMixin#noCubes$getVisualShape})
 ...
 * - Player should not suffocate when {@link SmoothTerrainConfig.Server#tempMobCollisionsDisabled} is false
...
public final class CollisionHandler {
	...
	public static VoxelShape getShapeOfSmoothBlock(BlockState state, BlockGetter reader, BlockPos blockPos, CollisionContext context) {
		var canCollide = ((ISmoothTerrainBlockType) state.getBlock()).noCubes$hasCollision();
		try {
	...
	public static VoxelShape getShapeOfSmoothBlockOrThrow(boolean canCollide, BlockState state, BlockGetter reader, BlockPos blockPos, EntityCollisionContext context) {
		if (!canCollide)
			return Shapes.empty();
		// assert SmoothTerrainConfig.Server.collisionsEnabled; // This is called from debug code & the ScreenEffectRenderer
		assert SmoothTerrain.smoothableHandler.isSmoothable(state);
...
//			return VoxelShapes.empty();
		...
		if (entity instanceof FallingBlockEntity || // Stop sand etc. breaking when it falls
			(SmoothTerrainConfig.Server.tempMobCollisionsDisabled && !(entity instanceof Player)) ||
		...
		// 26.2 removed Level#getProfiler; the current profiler is a static/thread-local.
		Profiler.get().push("SmoothTerrain collisions");
	...
	public static Deque<VoxelShape> createSmoothTerrainIntersectingCollisionList(CollisionGetter world, AABB area, MutableBlockPos pos) {
	...
	public static void generate(Area area, Mesher mesher, ShapeConsumer consumer) {
		mesher.generateCollisions(area, SmoothTerrain.smoothableHandler::isSmoothable, consumer);
	}
	...
	public static boolean generateShapes(Vec centre, Vec faceNormal, ShapeConsumer consumer, Face face) {
		...
		var mutOldZ = mut.z;
		try {
			if (!generateShapesOld(consumer, face, centre, mut, Double.MAX_VALUE, 0.125F, SmoothTerrainConfig.Server.oldStyleCollisionsEnhancementLevel))
				return false;
		} finally {
			mut.set(
				mutOldX,
		...
		if (!SmoothTerrainConfig.Server.onlyOldStyleCollisions) {
