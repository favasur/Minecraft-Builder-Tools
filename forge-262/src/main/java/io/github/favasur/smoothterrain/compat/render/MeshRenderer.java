...
import io.github.favasur.smoothterrain.mesh.OldSmoothTerrain;
...
import static io.github.favasur.smoothterrain.mesh.Mesher.getMeshOffset;
import static net.minecraft.world.level.block.SnowyBlock.SNOWY;

/**
 * 26.2 adaptation of the canonical mesh generator. Differences from 1.21.1: {@code isSolidRender}
 * is now 0-arg, {@code SnowyDirtBlock} was renamed to {@link SnowyBlock}, and the 26.2
 * {@code BlockAndTintGetter} has no {@code getShade} so directional face shading is computed with
 * the fixed vanilla factors. The quad extraction itself moved to {@link VanillaRenderer} (26.2's
 * {@code BlockStateModel}/{@code BlockStateModelPart} API), so the per-quad helpers from 1.21.1
 * are not present here.
 */
public class MeshRenderer {

	static boolean shouldSkipChunkMeshRendering() {
		return SmoothTerrainConfig.Client.debugSkipSmoothTerrainRendering || !SmoothTerrainConfig.Client.render;
	}

	public interface ISmoothTerrainAreaRenderer {
		void quad(
	...
	public static boolean isSolidRender(BlockState state) {
		return state.isSolidRender() || state.getBlock() instanceof DirtPathBlock;
	}
	...
	public static boolean renderSingleBlock(
		...
		ISmoothTerrainAreaRenderer renderer
	) {
		var start = System.nanoTime();
		var smoothable = SmoothTerrain.smoothableHandler.isSmoothable(state);
		var plant = SmoothTerrainConfig.Client.fixPlantHeight && ModUtil.isShortPlant(state);
		if (!smoothable && !plant)
			return false;
	...
	public static void renderArea(
		...
		try (
			...
			runForSolidAndSeeThrough(isSmoothableIn, isSmoothable -> mesher.generateGeometry(area, isSmoothable, (ignored, face) -> {
				...
				if (mesher instanceof OldSmoothTerrain) {
				...
				renderer.quad(
					...
					null,
					lightCache, getFaceShade(faceInfo)
				);
	...
	private static float getFaceShade(FaceInfo faceInfo) {
		return switch (faceInfo.approximateDirection) {
			case DOWN -> 0.5F;
			case UP -> 1.0F;
	...
	private static void runForSolidAndSeeThrough(Predicate<BlockState> isSmoothable, java.util.function.Consumer<Predicate<BlockState>> action) {
		action.accept(state -> isSmoothable.test(state) && isSolidRender(state));
		action.accept(state -> isSmoothable.test(state) && !isSolidRender(state));
	}

	private static void renderExtras(
		ISmoothTerrainAreaRenderer renderer, MutableObjects objects,
		BlockAndTintGetter world,
		Area area, LightCache lightCache,
		RenderableState foundState, RenderableState renderState,
		FaceInfo faceInfo
	) {
		var renderPlantsOffset = SmoothTerrainConfig.Client.fixPlantHeight;
		var renderGrassTufts = SmoothTerrainConfig.Client.grassTufts;
		...
		if (renderGrassTufts && foundState.state.hasProperty(SNOWY) && !ModUtil.isPlant(stateAbove)) {
			...
			var offset = grass.getOffset(worldAbove);
			...
			renderer.quad(
				...
				colorOverride,
				lightCache, getFaceShade(grassTuft)
			);
	...
	static final class RenderableState {
		...
		public static RenderableState findRenderFor(MutableObjects objects, RenderableState foundState, Area area, Direction faceDirection) {
			boolean tryFindSnow = faceDirection == Direction.UP || SmoothTerrainConfig.Client.moreSnow;
			if (!tryFindSnow)
				return foundState;
