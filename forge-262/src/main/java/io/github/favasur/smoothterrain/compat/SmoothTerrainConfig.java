...
import io.github.favasur.smoothterrain.mesh.*;
...
/**
 * The Config for SmoothTerrain.
 * Contains the Common, Client and Sever configs as inner classes.
 *
 * @author Cadiboo
 */
public final class SmoothTerrainConfig {
	...
	public static class Client {
		...
		public static boolean debugSkipSmoothTerrainRendering;

		public static int hashChunkRenderSettings() {
			return Objects.hash(SmoothTerrainConfig.Client.betterGrassSides, SmoothTerrainConfig.Client.moreSnow, SmoothTerrainConfig.Client.fixPlantHeight, SmoothTerrainConfig.Client.grassTufts);
		}
	}
	...
	public static class Server {
		...
		public static boolean oldSmoothTerrainSlopes;
		public static boolean oldSmoothTerrainInFluids;
		public static float oldSmoothTerrainRoughness;

		public enum MesherType {
			SurfaceNets(new SurfaceNets(false)),
			OldSmoothTerrain(new OldSmoothTerrain()),
		...
		public static int hashChunkRenderSettings(Stream<Block> blocks) {
			var smoothables = blocks
				...
				.map(SmoothTerrain.smoothableHandler::isSmoothable)
				.toArray(Boolean[]::new);
			return Objects.hash(SmoothTerrainConfig.Server.mesher, SmoothTerrainConfig.Server.forceVisuals, Arrays.hashCode(smoothables));
		}
	}
	...
	public static class Smoothables {
		...
		static {
			...
			DEFAULT_SMOOTHABLES.addAll(Arrays.stream(new Block[]{
				...
				BONE_BLOCK,
				DIRT_PATH,
				// 26.2 consolidated the colored terracotta into Blocks.DYED_TERRACOTTA.
				CLAY, TERRACOTTA,
				DYED_TERRACOTTA.pick(DyeColor.WHITE), DYED_TERRACOTTA.pick(DyeColor.ORANGE), DYED_TERRACOTTA.pick(DyeColor.MAGENTA), DYED_TERRACOTTA.pick(DyeColor.LIGHT_BLUE), DYED_TERRACOTTA.pick(DyeColor.YELLOW), DYED_TERRACOTTA.pick(DyeColor.LIME), DYED_TERRACOTTA.pick(DyeColor.PINK), DYED_TERRACOTTA.pick(DyeColor.GRAY), DYED_TERRACOTTA.pick(DyeColor.LIGHT_GRAY), DYED_TERRACOTTA.pick(DyeColor.CYAN), DYED_TERRACOTTA.pick(DyeColor.PURPLE), DYED_TERRACOTTA.pick(DyeColor.BLUE), DYED_TERRACOTTA.pick(DyeColor.BROWN), DYED_TERRACOTTA.pick(DyeColor.GREEN), DYED_TERRACOTTA.pick(DyeColor.RED), DYED_TERRACOTTA.pick(DyeColor.BLACK),
		...
		public static void updateUserDefinedSmoothableStringLists(boolean newValue, BlockState[] states, List<String> whitelist, List<String> blacklist) {
			...
			for (var state : states) {
				...
				SmoothTerrain.smoothableHandler.setSmoothable(newValue, state);
		...
		public static void recomputeInMemoryLookup(Stream<Block> blocks, List<? extends String> whitelist, List<? extends String> blacklist, boolean useDefaultSmoothables) {
			...
			blocks.parallel()
				...
				.forEach(state -> {
					...
					if (Common.debugEnabled && SmoothTerrain.smoothableHandler.isSmoothable(state) != smoothable)
						LOG.debug(() -> "Updating smoothness of %s to %b".formatted(state, smoothable));
					SmoothTerrain.smoothableHandler.setSmoothable(smoothable, state);
				});
		}
