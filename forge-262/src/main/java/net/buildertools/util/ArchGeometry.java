...
public final class ArchGeometry {
     ...
     * OUTWARD (verified against the analytic outward direction in {@link #outwardNormal}).
    ...
    public static List<BakedQuad> wedgeQuads(ArchBlockData a, BlockState state) {
        ...
        for (int f = 0; f < 6; f++) {
            ...
            Vec3 p2 = v[face[2]];
            Vec3 p3 = v[face[3]];
            BakedQuad.MaterialInfo info = materialFor(state, dirs[f]);
            if (info == null || info.sprite() == null) {
                continue;
            }
            Direction cull = nearestDirection(outwardNormal(p0, p1, p2));
            quads.add(new BakedQuad(
                    vec(p0), vec(p1), vec(p2), vec(p3),
                    uv(info.sprite(), (float) uvs[f][0], (float) uvs[f][1]),
                    uv(info.sprite(), (float) uvs[f][2], (float) uvs[f][3]),
                    uv(info.sprite(), (float) uvs[f][4], (float) uvs[f][5]),
                    uv(info.sprite(), (float) uvs[f][6], (float) uvs[f][7]),
                    cull, info));
        }
        return quads;
    }
    ...
    static long uv(TextureAtlasSprite sprite, float uTiles, float vTiles) {
        return UVPair.pack(sprite.getU(uTiles), sprite.getV(vTiles));
    }
