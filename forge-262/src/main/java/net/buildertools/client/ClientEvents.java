...
import net.minecraft.client.DeltaTracker;import com.mojang.blaze3d.platform.InputConstants;
...
public final class ClientEvents {
    ...
    public static boolean onMouseButtonPre(InputEvent.MouseButton.Pre event) {
        ...
        if (item instanceof SelectionToolItem && event.getButton() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            ...
            if (event.getButton() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                cancelled = handleEllipseLmbPress(player, minecraft);
            ...
            if (event.getButton() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                cancelled = handleArchLmbPress(player, minecraft);
            } else if (event.getButton() == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
                // Plain placement: let vanilla place the block; onRightClickBlock records it.
            }
     ...
     * for a floor/ceiling one); the region's projected extents define its size. Returns true
     * when the press was consumed.
     */
    private static boolean handleEllipseLmbPress(Player player, Minecraft minecraft) {
        HitResult hit = minecraft.hitResult;
     ...
     * along the in-plane rise axis defines the Rise, the row defines the Span. Returns true when
     * the press was consumed.
     */
    private static boolean handleArchLmbPress(Player player, Minecraft minecraft) {
