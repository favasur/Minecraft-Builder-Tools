...
/**
 * 26.2 adaptation of the canonical SmoothTerrain keybindings. The 26.2 copy has no SmoothTerrain
 * network channel (Builder Tools' world-setting toggle owns enable/disable), so smoothable toggles
 * are applied locally, matching the canonical 'server without SmoothTerrain' fallback path.
 */
public final class KeyMappings {
	...
	private static final KeyMapping.Category CATEGORY = KeyMapping.Category.register(
		Identifier.fromNamespaceAndPath(SmoothTerrain.MOD_ID, "smoothterrain"));
	...
	private static Pair<KeyMapping, Runnable> makeKeybinding(Consumer<KeyMapping> registerKey, String name, int key, Runnable action) {
		...
		var mapping = new KeyMapping(qualifyName(name), InputConstants.Type.KEYSYM, key, CATEGORY);
	...
	private static void toggleVisuals() {
		...
		ClientUtil.updateClientVisuals(!SmoothTerrainConfig.Client.render);
	...
	private static void toggleLookedAtSmoothable(boolean changeAllStatesOfBlock) {
		...
		// 26.2 has no SmoothTerrain network channel; the toggle is applied locally.
		SmoothTerrain.smoothableHandler.setSmoothable(newValue, states);
		reloadAllChunks("toggleLookedAtSmoothable was pressed");
	}
