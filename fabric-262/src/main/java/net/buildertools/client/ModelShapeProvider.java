...
public final class ModelShapeProvider {
    ...
    private static Face faceOf(BakedQuad quad) {
        // 26.2 quad positions are in model units (0..16); normalize to 0..1.
        double scale = Math.max(Math.max(Math.abs(quad.position(0).x()), Math.abs(quad.position(0).y())),
                Math.abs(quad.position(0).z())) > 1.5 ? 1.0 / 16.0 : 1.0;
        double[][] p = new double[4][3];
        for (int i = 0; i < 4; i++) {
            p[i][0] = quad.position(i).x() * scale;
            p[i][1] = quad.position(i).y() * scale;
            p[i][2] = quad.position(i).z() * scale;
        }
        int axis = -1;
        double value = 0;
        for (int a = 0; a < 3; a++) {
        ...
        int sign = quad.direction().getAxisDirection() == Direction.AxisDirection.POSITIVE ? 1 : -1;
