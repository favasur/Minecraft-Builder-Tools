...
public final class CreativeSettingsScreen extends CreativeModeInventoryScreen {
    ...
    protected void init() {
        ...
        controls.add(new Slider("Time of Day", 0, 24000, 50,
                this.player.level().getDefaultClockTime() % 24000,
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(this.font, "Creative Settings", panelX + 12, panelY + 10, 0xFFFFFFFF);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean inside) {
        ...
        if (mouseX >= panelX && mouseX <= panelX + panelW && mouseY >= panelY && mouseY <= panelY + panelH) {
            ...
            for (Control control : controls) {
                if (control.mouseClicked(mouseX, adjustedY, event.button())) {
                    return true;
                }
            }
            return true;
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        ...
        double mouseY = event.y();
        for (Control control : controls) {
            control.mouseDragged(mouseX, mouseY + scrollOffset, event.button(), dragX, dragY);
        }
        if (mouseX >= panelX && mouseX <= panelX + panelW && mouseY >= panelY && mouseY <= panelY + panelH) {
            return true;
        }
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        for (Control control : controls) {
            control.mouseReleased(event.x(), event.y() + scrollOffset, event.button());
        }
    ...
    private final class Header implements Control {
        ...
        public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
            graphics.text(font, text, x, y, 0xFF7FA8E0);
    ...
    private final class Slider implements Control {
        ...
        @Override
        public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
            graphics.text(font, label, x, y, 0xFFB8C7D9);
    ...
    private final class Toggle implements Control {
        ...
        @Override
        public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
            graphics.text(font, label, x, y + 3, 0xFFB8C7D9);
            ...
            graphics.text(font, on ? "ON" : "OFF", bx + (20 - font.width(on ? "ON" : "OFF")) / 2, y + 2, 0xFFFFFFFF);
        }
    ...
    private final class Button implements Control {
        ...
        public void render(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
            ...
            graphics.text(font, label, tx, y + (h - 8) / 2, hovered ? 0xFFFFFFFF : 0xFFB8C7D9);
        }
