...
public final class FreeBlockRaycast {
    ...
    private static Direction triangleSide(MeshCollisionShape.Tri t, Vec3 from, Vec3 to) {
        ...
        return nearestAxis(nx / len, ny / len, nz / len);
    }

    /** 26.2 removed {@code Direction.getNearest(double,double,double)} - pick the dominant axis. */
    private static Direction nearestAxis(double nx, double ny, double nz) {
        ...
        if (ay >= az) {
            return ny >= 0.0 ? Direction.UP : Direction.DOWN;
        }
