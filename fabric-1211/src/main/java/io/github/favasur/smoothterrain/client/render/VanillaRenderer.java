...
/**
 * Fabric 1.21.1 port of the canonical SmoothTerrain chunk renderer. The canonical build injects
 * the mesh through NeoForge's {@code AddSectionGeometryEvent}; Fabric has no such event, so the
 * mesh is drawn by {@code SectionCompilerMixin} at the exact point the vanilla per-block loop
 * finishes and before the section's buffers are built into MeshData. Everything downstream of
 * {@link #renderChunk} is identical to the canonical renderer; only the entry signature and the
 * vanilla (7-arg, no ModelData/RenderType) {@code BlockRenderDispatcher#renderBatched} differ.
 */
public final class VanillaRenderer {
	...
	public static void renderChunk(BlockPos sectionOrigin, PoseStack matrix, BlockAndTintGetter world,
		...
		// vanilla block pipeline with the same buffer/pose context as the section compile.
	...
	static void renderInBlockLayers(
		...
		// Fabric has no model data; the quad getter ignores it and vanilla renderBatched is used.
