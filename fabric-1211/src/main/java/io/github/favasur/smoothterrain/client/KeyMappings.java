...
/**
 * Fabric adaptation of the canonical SmoothTerrain keybindings. The canonical class sends
 * smoothable toggles to the server through the NeoForge payload channel; Fabric's Builder Tools
 * bundle has no SmoothTerrain network layer, so toggles are applied locally (singleplayer and
 * LAN-style use, matching the 'server without SmoothTerrain' fallback path of the canonical code).
 */
