...
import io.github.favasur.smoothterrain.mesh.SDFMesher.CollisionObjects;
...
public final class OldSmoothTerrain extends SimpleMesher {
	...
	private void generateInternal(Area area, Predicate<BlockState> isSmoothable, FullBlockAction fullBlockAction, FaceAction faceAction) {
		...
		var roughness = SmoothTerrainConfig.Server.oldSmoothTerrainRoughness;
		iterateSmoothBlocksInsideMesh(area, isSmoothable, (x, y, z, index) -> {
			...
			for (byte pointIndex = 0; pointIndex < 8; ++pointIndex) {
				...
				if (!doesPointIntersectWithSolid(area, point, isSmoothable)) {
					if (SmoothTerrainConfig.Server.oldSmoothTerrainSlopes) {
	...
	/**
	 * Checks if the state should be connected to with a slope (rather than a full side) when it is next to a smooth block if {@link SmoothTerrainConfig.Server#oldSmoothTerrainSlopes} is enabled.
	 * Also used to decide if parts of a neighbouring smoothable block can extend into it if {@link SmoothTerrainConfig.Server#oldSmoothTerrainRoughness} is enabled.
	 */
	public static boolean isNonSolid(BlockState state) {
		return state.isAir() || (SmoothTerrainConfig.Server.oldSmoothTerrainInFluids && state.getBlock() instanceof LiquidBlock && state.getFluidState().isSource()) || ModUtil.isPlant(state) || ModUtil.isSnowLayer(state);
	}
